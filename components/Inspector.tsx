...
export const Inspector: React.FC<InspectorProps> = ({ 
  ...
  if (selectedJobs.length === 1) {
      ...
      const handleEnhance = async () => {
          ...
          setIsEnhancing(true);
          const enhanced = await enhancePrompt(job.localPrompt);
          if (enhanced) onUpdateJob(job.id, { localPrompt: enhanced });
          setIsEnhancing(false);
      };

      const handleAutoDraft = async () => {
          setIsAutoDrafting(true);
          // Pass Job ID to backend
          const desc = await generateImageDescription(job.id);
          if (desc) onUpdateJob(job.id, { localPrompt: desc });
          setIsAutoDrafting(false);
      };
      ...
      const handleSmartRename = async () => {
          setIsRenaming(true);
          // Pass Job ID to backend
          const name = await generateSmartFilename(job.id);
          if (name) {
              const ext = job.fileName.split('.').pop() || 'png';
              // Ensure name doesn't already have extension from AI
