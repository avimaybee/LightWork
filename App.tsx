    ...
    const processNext = async (workerId: number) => {
        ...
        try {
            ...
            if (result.success && result.imageBytes) {
                ...
                if (result.isRetryable && job.retryCount < 3) {
                    const delay = calculateBackoff(job.retryCount);
                    updateJob(job.id, { status: 'retrying', errorMsg: `${result.error} (Retry in ${Math.round(delay/1000)}s)`, retryCount: job.retryCount + 1 });
                    await wait(delay);
