...
// C:/Users/lenovo/AppData/Roaming/npm/node_modules/wrangler/node_modules/unenv/dist/runtime/_internal/utils.mjs
// @__NO_SIDE_EFFECTS__
...
// C:/Users/lenovo/AppData/Roaming/npm/node_modules/wrangler/node_modules/unenv/dist/runtime/node/internal/perf_hooks/performance.mjs
...
// C:/Users/lenovo/AppData/Roaming/npm/node_modules/wrangler/node_modules/@cloudflare/unenv-preset/dist/runtime/polyfill/performance.mjs
...
// C:/Users/lenovo/AppData/Roaming/npm/node_modules/wrangler/node_modules/unenv/dist/runtime/node/console.mjs
import { Writable } from "node:stream";

// C:/Users/lenovo/AppData/Roaming/npm/node_modules/wrangler/node_modules/unenv/dist/runtime/mock/noop.mjs
...
// C:/Users/lenovo/AppData/Roaming/npm/node_modules/wrangler/node_modules/@cloudflare/unenv-preset/dist/runtime/node/console.mjs
...
// C:/Users/lenovo/AppData/Roaming/npm/node_modules/wrangler/_virtual_unenv_global_polyfill-@cloudflare-unenv-preset-node-console
globalThis.console = console_default;

// C:/Users/lenovo/AppData/Roaming/npm/node_modules/wrangler/node_modules/unenv/dist/runtime/node/internal/process/hrtime.mjs
...
// C:/Users/lenovo/AppData/Roaming/npm/node_modules/wrangler/node_modules/unenv/dist/runtime/node/internal/process/process.mjs
import { EventEmitter } from "node:events";

// C:/Users/lenovo/AppData/Roaming/npm/node_modules/wrangler/node_modules/unenv/dist/runtime/node/internal/tty/read-stream.mjs
...
// C:/Users/lenovo/AppData/Roaming/npm/node_modules/wrangler/node_modules/unenv/dist/runtime/node/internal/tty/write-stream.mjs
...
// C:/Users/lenovo/AppData/Roaming/npm/node_modules/wrangler/node_modules/unenv/dist/runtime/node/internal/process/node-version.mjs
...
// C:/Users/lenovo/AppData/Roaming/npm/node_modules/wrangler/node_modules/@cloudflare/unenv-preset/dist/runtime/node/process.mjs
...
// C:/Users/lenovo/AppData/Roaming/npm/node_modules/wrangler/_virtual_unenv_global_polyfill-@cloudflare-unenv-preset-node-process
...
async function onRequestPost(context2) {
  try {
    const { type, jobId, text } = await context2.request.json();
    const ai = new GoogleGenAI({ apiKey: context2.env.API_KEY });
    ...
    if (type === "enhance") {
      ...
      const imageRecord = await context2.env.DB.prepare("SELECT r2_key_original FROM images WHERE id = ?").bind(jobId).first();
      if (!imageRecord) return new Response("Image not found", { status: 404 });
      const obj = await context2.env.STORAGE.get(imageRecord.r2_key_original);
...
async function onRequestPost2(context2) {
  try {
    ...
    await context2.env.STORAGE.put(r2Key, file.stream(), {
...
async function onRequestGet(context2) {
  ...
  try {
    const object = await context2.env.STORAGE.get(key);
    if (!object) {
...
async function onRequestDelete(context2) {
  ...
  await context2.env.DB.prepare("DELETE FROM modules WHERE id = ?").bind(id).run();
...
async function onRequestPatch(context2) {
  try {
    const id = context2.params.id;
    const updates = await context2.request.json();
    const fields = [];
    const values = [];
    ...
    if (updates.modulePrompt !== void 0) {
      fields.push("module_prompt = ?");
      values.push(updates.modulePrompt);
    }
    if (updates.selectedMode !== void 0) {
      fields.push("selected_mode = ?");
      values.push(updates.selectedMode);
    }
    if (updates.selectedModulePreset !== void 0) {
      fields.push("selected_module_preset = ?");
      values.push(updates.selectedModulePreset);
    }
    if (fields.length === 0) return new Response("No valid fields", { status: 400 });
    values.push(id);
    const query = `UPDATE jobs SET ${fields.join(", ")} WHERE id = ?`;
    await context2.env.DB.prepare(query).bind(...values).run();
    return new Response(JSON.stringify({ success: true }));
  } catch (e) {
    return new Response(e.message, { status: 500 });
  }
...
async function onRequestDelete2(context2) {
  const id = context2.params.id;
  const images = await context2.env.DB.prepare("SELECT r2_key_original, r2_key_result FROM images WHERE job_id = ?").bind(id).all();
  const keysToDelete = [];
  ...
  if (keysToDelete.length > 0) {
    await Promise.all(keysToDelete.map((key) => context2.env.STORAGE.delete(key)));
  }
  await context2.env.DB.batch([
    context2.env.DB.prepare("DELETE FROM images WHERE job_id = ?").bind(id),
    context2.env.DB.prepare("DELETE FROM jobs WHERE id = ?").bind(id)
  ]);
  return new Response(JSON.stringify({ success: true }));
}
...
async function onRequestGet2(context2) {
  try {
    const { results } = await context2.env.DB.prepare("SELECT * FROM modules").all();
    const modules = results.map((m) => ({
      id: m.id,
      name: m.name,
      prompt: m.prompt,
      isCustom: true
      // DB modules are custom or stored system ones.
    }));
...
async function onRequestPost3(context2) {
  try {
    const { name, prompt } = await context2.request.json();
    const id = crypto.randomUUID();
    await context2.env.DB.prepare(
...
async function onRequestPost4(context2) {
  ...
  try {
    ...
    const imageRecord = await context2.env.DB.prepare("SELECT * FROM images WHERE id = ?").bind(jobId).first();
    if (!imageRecord) return new Response("Image not found", { status: 404 });
    const obj = await context2.env.STORAGE.get(imageRecord.r2_key_original);
    ...
    if (isPro) {
      config2.imageConfig = {
        imageSize: "2K"
        // Default to high quality for Pro
      };
    }
    const response = await ai.models.generateContent({
...
${userPrompt || ""}` }
    ...
    await context2.env.STORAGE.put(resultKey, bytes, {
      httpMetadata: { contentType: "image/png" }
    });
    await context2.env.DB.prepare("UPDATE images SET status = ?, r2_key_result = ? WHERE id = ?").bind("completed", resultKey, jobId).run();
    return new Response(JSON.stringify({ success: true, imageBytes }));
  } catch (e) {
    try {
      if (jobId) {
        await context2.env.DB.prepare("UPDATE images SET status = ?, error_msg = ? WHERE id = ?").bind("error", e.message, jobId).run();
      }
    } catch (dbError) {
    }
    return new Response(JSON.stringify({ success: false, error: e.message }));
...
async function onRequestGet3(context2) {
  try {
    ...
    const projects = await Promise.all(results.map(async (job) => {
      const images = await context2.env.DB.prepare("SELECT * FROM images WHERE job_id = ?").bind(job.id).all();
      return {
        id: job.id,
        ...
        jobs: images.results.map((img) => ({
          ...
          resultUrl: img.r2_key_result ? `/api/images/${img.r2_key_result}` : void 0,
...
// ../.wrangler/tmp/pages-Abq25I/functionsRoutes-0.4600143901300633.mjs
var routes = [
  {
